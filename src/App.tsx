...
import { useSession, useProfile } from "./useQuota"
...
export default function App() {
  ...
  if (stytchToken !== null && search.get("stytch_token_type") !== null) {
    screen = <StytchCallback token={stytchToken} />
  } else
  switch (route.name) {
    case "home":
    ...
    case "new":
      ...
      break
    case "account":
      screen = session !== null ? <Account session={session} /> : <SignIn auth={auth} returnTo="/account" />
      break
    case "auth":
...
// now-stored session on reload). Lands on the console (returnTo isn't carried
// through Stytch's bare redirect).
function StytchCallback({ token }: { token: string }) {
